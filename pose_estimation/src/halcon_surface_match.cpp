...
namespace pose_estimation
{
  ...
  bool HalconSurfaceMatch::find_object_in_scene(std::string object, std::vector<float> &pose_estimate)
  {
    ...
    if(log) //check if file open
    {
      log << std::endl << "Found object: " << object << ", match score: ";
      std::string score_str = static_cast<std::string>(score.ToString());
      auto s = score_str.substr(0, score_str.find(",")) + "]";
      log << s;
      log.close();
    } 
